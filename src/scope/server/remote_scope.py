...
class RemoteScopeConnection:
    ...
    async def start_webrtc(self, initial_parameters: dict | None = None) -> None:
        ...
        params = build_remote_scope_initial_parameters(initial_parameters)
    ...
    async def api_request(
        ...
        async with aiohttp.ClientSession(headers=self._headers()) as session:
            if method.upper() == "POST" and urlparse(path).path == (
            ...
            if (
                ...
                and urlparse(path).path == "/api/v1/pipeline/load"
            ):
                logger.info(
...
def build_remote_scope_output_mapping(
    ...
    num_output_tracks = num_sink_slots + len(graph_info.remote_record_node_ids)
    num_local_handlers = num_sink_slots + len(graph_info.record_node_ids)
    remote_to_local = list(range(num_sink_slots))
...
def build_remote_scope_initial_parameters(
    ...
    params = rewrite_remote_scope_graph_for_runner(params, graph_info)
    return params
...
def rewrite_remote_scope_graph_for_runner(
    ...
    if not collapsed_records and not remote_records:
        return params
    ...
    if isinstance(edges, list) and collapsed_records:
        graph["edges"] = [
            edge
            for edge in edges
            if isinstance(edge, dict)
            and edge.get("to_node") not in collapsed_records
            and edge.get("from") not in collapsed_records
        ]
    return params
